...
class QueryAnalyzer:
    ...
    def analyze(self, query: str) -> AnalyzedQuery:
        ...
        if aq.query_type == "statistical":
            # 统计查询: 只保留 domain 过滤, 其他参数置空(走全量聚合)
            aq.category = None
            aq.voltage_level = None
            aq.publish_level = None
            aq.discipline = None
            aq.equipment_type = None
            aq.year = None

        elif aq.query_type == "specification_lookup":
    ...
    def _extract_publish_level(self, query: str) -> Optional[str]:
        ...
        if re.search(r'行标|DL|行业标准', query):
            return "行标"
        if re.search(r'省公司|省电力', query):
            return "省级"
        if re.search(r'市公司|供电公司', query):
            return "地市"
        return None
    ...
    def _is_statistical_query(self, query: str) -> bool:
        ...
        关键词分类:
          - 计数: 多少(份/个/条), 几个, 有几, 数量, 共计, 共有
          - 列举: 哪些, 列出, 所有.*文件, 有哪些
          - 分布: 分布, 占比, 分组, 按.*(统计|分类|分组), 各.*(分布|数量|几)
        """
        count_patterns = [
            r'多少[份个条项种张]', r'有多少[份个条项种张]',
            r'几个', r'有几[份个条项种张]', r'几[份个条项种张]',
            r'数量', r'共计', r'共有', r'一共',
        ]
        list_patterns = [
            r'哪些', r'列出', r'所有.*文件', r'有哪些',
            r'哪几', r'哪几个',
        ]
        dist_patterns = [
            r'分布', r'占比', r'分组', r'统计', r'汇总',
            r'按.*(?:统计|分类|分组)', r'各.*(?:分布|数量|几[份个])',
        ]
        all_patterns = count_patterns + list_patterns + dist_patterns
        return any(re.search(p, query) for p in all_patterns)

    def _extract_statistical_dimensions(self, query: str) -> dict:
        """从统计型查询中提取要聚合的维度"""
        dims = {}
        if re.search(r'域|专业领域|知识域', query):
            dims["domain"] = True
        if re.search(r'电压[等级]|kV|千伏|多少[Vv]', query):
            dims["voltage_level"] = True
        if re.search(r'类[目别]|分类|类别', query):
            dims["category"] = True
        if re.search(r'发布[层级位]|标准级别|国标|行标|企标', query):
            dims["publish_level"] = True
        if re.search(r'年[份度]|时间|年份', query):
            dims["year"] = True
        if re.search(r'专业[类型]|电气|土建|二次|通信', query):
            dims["discipline"] = True
        if re.search(r'设备|变压器|开关|电缆|断路器', query):
            dims["equipment_type"] = True
        return dims
