...
class Chunker:
    ...
    def chunk_page_text(self, text: str, page_num: int, total_pages: int,
                        file_meta: dict) -> List[Chunk]:
        """
        对单页文本分块 — 保留 page_num, 消除跨批 chunk_id 冲突

        短页 → 单 chunk (single_page)
        长页 → 页内语义拆分, chunk_id 带页码: {hash}_p{page}_{i}
        """
        text = (text or "").strip()
        if not text:
            return []
        ...
        if len(text) < self.max_chars_for_full:
            chunk = self._make_per_page_chunks(
                [{"page_num": page_num, "text": text}], file_meta
            )[0]
            chunk.total_pages = total_pages
            return [chunk]

        # 长页: 页内语义拆分
        max_chars = self.semantic_chunk_size * 3
        ...
        for i, split_text in enumerate(splits):
            if not split_text.strip():
                continue
            chunk = self._make_full_document_chunk(split_text, file_meta, total_pages)
            chunk.chunk_id = f"{file_meta['file_hash']}_p{page_num}_{i}"
            chunk.page_num = page_num
            chunk.chunk_index = i
            chunk.total_chunks = total
            chunk.chunk_strategy = "semantic"
            chunks.append(chunk)
        return chunks
